...
async def home():
    ...
    <head>
        ...
        <style>
            ...
            .mp4 { background: #ff0000; }
            .mp3 { background: #1db954; }
    ...
    <div class="box">
        ...
        <input type="text" id="url" placeholder="Pega enlace de YouTube">
        <button class="mp4" onclick="download('mp4')">Descargar MP4</button>
        <button class="mp3" onclick="download('mp3')">Descargar Audio</button>
        <div id="status"></div>
    </div>
    <script>
        function download(tipo) {
            ...
            if (!url) { status.innerHTML = "❌ Ingresa una URL"; return; }
            status.innerHTML = "⏳ Procesando...";
            const cleanUrl = url.split("&")[0];
            window.location.href = "/download?url=" + encodeURIComponent(cleanUrl) + "&tipo=" + tipo;
        }
    </script>
    </body>
    </html>
...
async def download_media(
    ...
    try:
        ...
        if not os.path.exists(filename):
            raise HTTPException(status_code=500, detail="No se encontró el archivo")
